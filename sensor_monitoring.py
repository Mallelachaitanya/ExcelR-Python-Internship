...
def is_co2_voilated(co2):
    ...
    if co2 < co2_ll or co2 > co2_ul:
        print(f'CO2 Alarm : CO2 is :{co2}, Lower Limit is {co2_ll}, Upper Limit is {co2_ul}')
        return True
    else:
...
def is_temp_voilated(temp):
    ...
    if temp < temp_ll or temp > temp_ul:
        print(f'Temperature Alarm : Temp is :{temp}, Lower Limit is {temp_ll}, Upper Limit is {temp_ul}')
        return True
    else:
...
def is_humidity_voilated(humidity):
    ...
    if humidity < humidity_ll or humidity > humidity_ul:
        print(f'Humidity Alarm : Humidity is :{humidity}, Lower Limit is {humidity_ll}, Upper Limit is {humidity_ul}')
        return True
    else:
